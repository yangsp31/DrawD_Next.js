...
export default function Home() {
  ...
  const handleLogin = () => {
    window.location.href = `https://${process.env.NEXT_PUBLIC_SERVER_DOMAIN}/oauth2/authorization/google`;
  };

  const handleLogOut = async() => {
    try {
      const response = await axios.get(`https://${process.env.NEXT_PUBLIC_SERVER_DOMAIN}/logout`, {
